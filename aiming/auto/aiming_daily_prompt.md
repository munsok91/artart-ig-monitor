...
- **구글드라이브 팀 검수함**: 아래 한 줄만 실행하면 된다 (드라이브 앱/rclone 중 되는 걸 알아서 골라 올리고, 둘 다 없으면 조용히 건너뛴다):
  ```sh
  bash scripts/upload_drive.sh episodes/epNN_주제 "[에이밍] YYYY-MM-DD_epNN_주제"
  ```
  폴더명은 **`[에이밍] 날짜_회차`** 형식 필수 (예: `[에이밍] 2026-07-13_ep07_금리인하`). 출력 로그를 요약에 포함.
