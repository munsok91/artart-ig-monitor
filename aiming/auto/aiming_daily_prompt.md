...
## 5) 인스타 발행 금지
- 이 자동 작업의 필수 완료 범위는 **카드뉴스 제작 → 아트아트 투데이 Drive 저장**까지다. Slack 연결이 있는 환경에서만 검수 링크를 추가로 알린다.
...
## 7) 상태 기록
`episodes/.covered_topics.json` 에 오늘 항목 `{"date","topic","episode","posted":false}` 를 추가(최근 60개 유지). 마지막에 1~3줄 요약 출력: 선정 소재·제외한 후보 이유·제작 결과. Drive 업로드는 바깥 러너가 이어서 처리한다고 명시한다.
